...
const PageElementsList2: React.FC<PageElementsListProps> = ({
  id,
  pageName,
  loadData,
}) => {
  ...
  useEffect(() => {
    ...
    const fetchPageElements = async () => {
      try {
        ...
        const response = await fetch(
          `${apiUrl}/seller/elementByIdAndPage/${id}/${pageName}/${user.slug}`,
          {
            credentials: "include",
          }
        );
    ...
    fetchPageElements();
  }, [id, pageName, change, loadData, user.slug]);

  // Update element status
  const handleToggle = async (elementId: string, currentStatus: boolean) => {
    try {
      const response = await fetch(
        `${apiUrl}/seller/updateStatus/${elementId}`,
        {
  ...
  const handleDelete = async (elementId: string) => {
    try {
      const response = await fetch(`${apiUrl}/seller/delete/${elementId}`, {
        method: "DELETE",
        credentials: "include",
      });
...
export default PageElementsList2;
