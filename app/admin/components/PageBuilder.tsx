...
const PageBuilder: React.FC<PageProps> = ({ onClose, page, id }) => {
  ...
  const [selectedOption, setSelectedOption] = useState(""); // Tracks whether category or writer is selected

  // Fetch banners, categories, and writers when the component mounts
  ...
  const handleChange = (
    ...
    // For selecting category or writer, update the productSectionId
    if (name === "productSectionId") {
      setFormData((prevFormData) => ({
        ...prevFormData,
        productSectionId: value, // Set either categoryId or writerId
      }));
    } else {
  ...
  const handleSelectionChange = (selection: string) => {
    ...
    // Reset the productSectionId when changing between category, subcategory, and writer
    setFormData((prevFormData) => ({
      ...prevFormData,
          ...
          <form onSubmit={handleSubmit}>
                ...
                <div className="flex space-x-4 mb-4">
                  ...
                  <label className="inline-flex items-center">
                    <input
                      ...
                      name="selection"
                      value="writer"
                      onChange={() => handleSelectionChange("writer")} // Use a function to handle selection change
                      checked={selectedOption === "writer"}
                      className="form-radio h-5 w-5"
                    />
                    <span className="ml-2 text-gray-700">Writer</span>
                  </label>
                </div>
              </div>
            )}
            <>
              ...
              {selectedOption === "writer" && (
                <div>
                  <label className="block" htmlFor="writerSelect">
                    Select Writer
                  </label>
                  <select
                    id="writerSelect"
