...
const Banner: React.FC<BannerProps> = ({ items, onDelete }) => {
    ...
    <>
      <div>
        {sortedItems?.map((item, index) => (
          <div
            className="container my-2 border border-gray-400 p-1 "
            key={index}
          >
            <div className="flex items-center justify-between mb-2">
              <div>
                ...
                <p>{item.subTitle ? item.subTitle : "No sub title"}</p>
              </div>
            </div>
            ...
            <p className="mt-12 flex justify-end gap-4">
              <Link
                className="border-green-600 rounded-lg border p-2"
                href={`/admin/banner/${item._id}`}
              >
                Edit{" "}
              </Link>
              <button
                className="border-red-600 rounded-lg border p-2"
                onClick={() => onDelete(item._id)} // Call onDelete prop
              >
                Delete
              </button>
            </p>
